...
export default function CvPage() {
  ...
  return (
    <div>
      ...
      {/* ── Uploader (visible si no hay CVs o el usuario lo pidió) ── */}
          ...
          {resumes.map((r) => {
            ...
            return (
              <Link
                key={r.id}
                href={`/cv/${r.id}`}
                className={`flex items-center justify-between border rounded-2xl px-5 py-4 transition-all hover:bg-white/5 ${scoreBg(score)}`}
              >
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-0.5">
                      <span className="text-xs text-white/30 capitalize">{r.kind}</span>

...
export default async function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen flex bg-[#0f1117]">
      {/* Sidebar */}
      <aside className="w-60 bg-[#13151f] border-r border-white/5 flex flex-col fixed h-full z-10">
        {/* Logo */}
        ...
        <nav className="flex-1 px-3 py-4 space-y-0.5">
          {navItems.map((item) => (
            ...
            >
              <span className="text-white/40 group-hover:text-violet-400 transition-colors">{item.icon}</span>
              {item.label}
            </Link>
          ))}
        </nav>
